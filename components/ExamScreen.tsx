...
import { ExamSession, UserAnswer, CodingQuestion, MCQQuestion, RunResult, SectionType } from '../types';
    ...
    if (isCoding && currentQuestion) {
      ...
      if (!existingAns?.codeStates?.[selectedLang]) {
        ...
        const newAnswer = { 
          ...
          runResult: existingAns?.runResult // Preserve result if exists
        };
        const updated = { ...answers, [qId]: newAnswer };
        setAnswers(updated);
        onUpdateAnswers(updated);
      } else {
        const newAnswer = { ...existingAns, answer: existingAns.codeStates![selectedLang], language: selectedLang };
        const updated = { ...answers, [qId]: newAnswer };
        setAnswers(updated);
        onUpdateAnswers(updated);
      }
  ...
  return (
    <div className="h-screen flex flex-col bg-[#020617]">
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-72 bg-[#020617] border-r border-white/5 p-6 space-y-2 shrink-0 overflow-y-auto">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          <div className="w-full md:w-1/2 p-12 overflow-y-auto border-r border-white/5 bg-slate-950/20 backdrop-blur-sm">
            {isCoding ? (
              <div className="max-w-none space-y-12">
                ...
                <div className="bg-slate-900/50 border border-white/10 rounded-3xl p-8 space-y-4">
                  <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Technical Constraints</h4>
                  <p className="text-xs font-mono text-slate-400 italic">{(currentQuestion as CodingQuestion).constraints || "Standard memory and time complexity requirements apply."}</p>
                </div>
                      ...
                      <div className="p-8 space-y-6">
                        <div className="grid grid-cols-2 gap-10">
                          <div>
                             ...
                             <pre className="p-4 bg-black/60 rounded-2xl text-slate-300 font-mono text-xs border border-white/5">{s.input}</pre>
                          </div>
                          <div>
                             <div className="text-[10px] font-black text-emerald-400 mb-2 uppercase tracking-widest opacity-50">Expected Output</div>
                             <pre className="p-4 bg-black/60 rounded-2xl text-slate-300 font-mono text-xs border border-white/5">{s.output}</pre>
                          </div>
                        </div>
                        {s.explanation && (
                          <div className="pt-4 border-t border-white/5">
                            <div className="text-[10px] font-black text-slate-500 mb-2 uppercase tracking-widest">Logic Breakdown</div>
                            <p className="text-sm text-slate-400 leading-relaxed font-medium">{s.explanation}</p>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="flex-1 flex flex-col bg-black overflow-hidden relative">
            {isCoding ? (
              <>
                <div className="flex items-center justify-between px-8 py-5 bg-[#020617] border-b border-white/5">
                  <div className="flex gap-1.5 bg-black/60 p-1.5 rounded-2xl border border-white/5">
                    {SUPPORTED_LANGUAGES.map(l => (
                      <button 
                        key={l.id} 
                        onClick={() => setSelectedLang(l.id)} 
                        className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedLang === l.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-600 hover:text-white hover:bg-white/5'}`}
                      >
                        {l.name}
                      </button>
                    ))}
                ...
                {activeRunResult && (
                  <div className="h-72 bg-[#020617] overflow-y-auto p-8 font-mono border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
                    <div className="flex justify-between items-center mb-8">
                       ...
                       <div className="flex items-center gap-6">
                         ...
                         <div className="text-[11px] font-black text-white bg-slate-900 px-4 py-1.5 rounded-xl border border-white/10">
                           {activeRunResult.score}% EFFICIENCY
                         </div>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-15 gap-3 mb-10 h-3">
                       {activeRunResult.testCaseResults.map((tr, i) => (
                         <div 
                          key={i} 
                          title={`Test ${i+1}: ${tr.passed ? 'PASSED' : 'FAILED'}`}
                          className={`h-full rounded-full transition-all duration-700 ${tr.passed ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.4)]' : 'bg-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.4)] animate-pulse'}`}
                         ></div>
                       ))}
                    </div>

                    <div className="space-y-4">
                       {activeRunResult.testCaseResults.filter(t => !t.passed).slice(0, 2).map((tr, i) => (
                         <div key={i} className="text-xs p-5 rounded-3xl bg-slate-950 border border-rose-500/20 relative overflow-hidden">
                            ...
                            <div className="flex justify-between mb-2">
                               ...
                               <span className="text-slate-700 font-black uppercase text-[9px] tracking-widest">{tr.category}</span>
                            </div>
                            <div className="text-slate-400 font-medium leading-relaxed italic">
                               {tr.isHidden ? "HIDDEN VECTOR BREACHED: Implementation failed a boundary scenario. Check edge scaling." : `Mismatch on input prefix: ${tr.input.slice(0, 40)}...`}
                            </div>
                         </div>
                       ))}
                       {activeRunResult.passed && (
                         <div className="text-xs p-6 rounded-3xl bg-emerald-500/5 border border-emerald-500/20 relative overflow-hidden">
                            <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500"></div>
                            <span className="font-black text-emerald-500 uppercase tracking-widest text-[10px]">Integrity Verified</span>
                            <p className="text-slate-400 mt-3 font-medium leading-relaxed italic">All 15 verification vectors processed successfully.</p>
                         </div>
                       )}
                    </div>
                  </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-[#020617]">
                ...
                <p className="text-slate-500 text-sm max-w-sm font-medium">Monitoring patterns on Neon persistence layer.</p>
              </div>
            )}
            
            <div className="p-10 bg-black/60 border-t border-white/5 flex justify-between gap-6 backdrop-blur-xl">
